...
const AdminLayout = () => {
  ...
  return (
    <div className={`flex h-screen font-sans text-base relative transition-colors duration-300 overflow-hidden ${darkMode ? 'bg-black text-gray-100' : 'bg-white text-gray-900'}`}>
      {/* Theme Toggle Button */}
      <button
      ...
      >
        {darkMode ? (
          ...
          <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-moon"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
        )}
      </button>
      ...
      >
        <button
          ...
          className="absolute -right-3 top-6 w-6 h-6 bg-gray-800 rounded-full flex items-center justify-center text-gold hover:bg-gray-700 transition-colors border border-gray-700 cursor-pointer z-50"
        ...
        <div className={`px-4 py-6 border-t ${darkMode ? 'border-gray-800' : 'border-gray-200'} flex items-center transition-all duration-300`}>
          <div className="group relative">
            <div className={`w-10 h-10 rounded-full ${darkMode ? 'bg-gray-700 text-gold hover:bg-gray-600' : 'bg-gray-200 text-yellow-700 hover:bg-gray-300'} flex items-center justify-center text-lg font-bold`}>
          ...
          {sidebarOpen && (
            <button
              onClick={logout}
              className={`ml-4 px-4 py-2 rounded-lg font-semibold transition ${darkMode ? 'bg-gray-900 text-red-400 hover:bg-red-900 hover:text-white' : 'bg-gray-100 text-red-600 hover:bg-red-200 hover:text-red-900'}`}
            >
              Logout
            </button>
          )}
