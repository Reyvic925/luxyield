...
async function verify() {
  try {
    ...
    if (!user) {
      console.log('âŒ No users found');
      process.exit(1);
    }
    ...
    console.log('ðŸ“‹ Test User Account Status:');
    ...
    if (user.availableBalance > 0) {
      console.log(`âœ… PASS: User has withdrawable balance of $${user.availableBalance.toFixed(2)}`);
      console.log(`âœ… PASS: ROI withdrawal is now ENABLED`);
      console.log(`âœ… PASS: User can withdraw ROI without issue`);
      console.log('\nðŸŽ‰ ROI WITHDRAWAL FIX IS WORKING!\n');
    } else {
      console.log(`âŒ FAIL: Available balance is still $0.00`);
      console.log(`âŒ FAIL: ROI is not being credited to available balance`);
    }
